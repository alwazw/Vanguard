  ...
  return (
    <section ref={containerRef} className="py-32 px-12 bg-deep-black border-y border-white/5">
      <div className="max-w-7xl mx-auto mb-24 flex flex-col md:flex-row items-end justify-between">
        <div>
            <span className="text-neon-lime font-mono text-sm tracking-[0.4em] uppercase mb-4 block animate-pulse">Impact Visualizer</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-1 max-w-7xl mx-auto bg-white/5">
        {cases.map((c) => (
          <div
            key={c.title}
            className="case-card group p-12 bg-deep-black relative overflow-hidden flex flex-col justify-between h-[600px] border border-white/5 transition-all duration-500 hover:border-neon-lime/30"
          >
            {/* Header */}
            <div>
              <div className="flex justify-between items-center mb-10">
                  <span className="text-xs font-mono font-bold uppercase tracking-[0.2em] text-neon-lime">{c.type}</span>
                  <div className="w-1.5 h-1.5 bg-neon-lime animate-pulse" />
              </div>
            ...
            {/* Bottom Section */}
            <div>
              <div className="text-5xl font-black mb-4 text-white group-hover:text-neon-lime transition-colors duration-300">{c.cost}</div>
