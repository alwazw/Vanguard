        ...
        {type === 'flipper' && (
          <group>
            ...
            <mesh position={[0, 0, 0.31]}>
              ...
              <meshStandardMaterial color="#B6FF3B" emissive="#B6FF3B" emissiveIntensity={0.5} wireframe />
            </mesh>
          </group>
        )}
        {type === 'cable' && (
           <TorusKnot args={[1, 0.05, 128, 16]} scale={1.2}>
             <meshStandardMaterial color="#B6FF3B" wireframe />
           </TorusKnot>
        )}
        {type === 'pineapple' && (
  ...
  return (
    <section ref={containerRef} className="h-screen overflow-hidden bg-deep-black py-20 border-t border-white/5">
      <div className="absolute top-24 left-16 z-10">
        <span className="text-neon-lime font-mono text-sm tracking-[0.4em] uppercase mb-4 block animate-pulse">Asset Repository</span>
        ...
        {arsenalItems.map((item) => (
          ...
          >
            ...
            <div className="absolute inset-0 opacity-5 pointer-events-none"
                 style={{backgroundImage: 'linear-gradient(45deg, #B6FF3B 1px, transparent 1px)', backgroundSize: '40px 40px'}} />
            ...
            <div className="relative z-10 w-full">
              <span className="text-neon-lime font-mono text-xs tracking-widest mb-4 block">{item.code}</span>
              <h3 className="text-4xl md:text-5xl font-black mb-6 uppercase tracking-tight text-white group-hover:text-neon-lime transition-colors duration-300">
                {item.name}
              </h3>
            ...
            <button className="relative z-10 px-10 py-4 border border-white/20 text-white hover:bg-neon-lime hover:border-neon-lime transition-all uppercase font-bold tracking-widest text-xs flex items-center gap-4">
