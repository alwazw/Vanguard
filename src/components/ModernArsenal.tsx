    ...
    <section ref={containerRef} className="py-16 md:py-24 px-4 md:px-6 bg-deep-black">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
          {/* Image */}
          <div className="relative h-64 md:h-96 lg:h-[500px] order-2 lg:order-1">
            <Image
              src="/images/hardware-tools.jpg"
              alt="Modern Arsenal - Penetration Testing Hardware"
              fill
              className="object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-r from-deep-black via-transparent to-transparent"></div>
          ...
          <div className="order-1 lg:order-2">
            <div className="inline-block px-3 py-1 border border-vanguard-red/30 bg-vanguard-red/5 mb-4">
              <span className="text-vanguard-red text-xs font-mono uppercase tracking-widest">Hardware Division</span>
            </div>
            <h2 className="text-3xl md:text-5xl font-bold uppercase mb-4 tracking-tighter">
              The Modern <span className="text-vanguard-red">Arsenal</span>
            </h2>
            ...
            <div className="space-y-3">
              {tools.map((tool) => (
                <div 
                  key={tool.id}
                  className="arsenal-item border border-white/10 bg-noir-dark overflow-hidden"
                >
                  <button
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-vanguard-red/10 border border-vanguard-red/30 flex items-center justify-center">
                        <svg className="w-5 h-5 text-vanguard-red" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z" />
                        </svg>
                      </div>
                      <span className="text-sm md:text-base font-bold group-hover:text-vanguard-red transition-colors">{tool.name}</span>
                    </div>
                    ...
                    <div className="px-4 md:px-5 pb-4 md:pb-5 pt-0">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tool.capabilities.map((cap) => (
                          <span key={cap} className="px-2 py-1 text-[10px] md:text-xs bg-vanguard-red/10 text-vanguard-red border border-vanguard-red/30">
                            {cap}
                          </span>
                        ))}
                      </div>
